...
export const metadata: Metadata = {
  title: "Collection",
  description: "View collection details and articles.",
};
...
export default async function CollectionDetailPage({
  ...
  const { data: collectionData, error: collectionError } = await supabase
    .from("collections")
    .select("id, title, slug, description, cover_image_url, published_at, created_at")
    .eq("slug", slug)
  ...
  if (collectionError || !collectionData) {
    return (
      <div className="mx-auto w-full max-w-4xl space-y-6">
        ...
        <Link href="/collections" className="text-sm text-zinc-600 hover:underline dark:text-zinc-400">
          ← Back to collections
        </Link>
      </div>
    );
  ...
  const { data: articleRows } = await supabase
    ...
    .select(`
      position,
      article:articles!collection_articles_article_id_fkey(
        id,
        title,
        slug,
        abstract,
