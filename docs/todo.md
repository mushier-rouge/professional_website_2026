...
| T-003 | Provision Vercel project via CLI | completed | Vercel CLI login + project linked (see `.vercel/project.json`). |
| T-004 | Provision Supabase project via CLI | blocked | Need `SUPABASE_ACCESS_TOKEN` to link project and push migrations. |
...
| T-007 | Document project identifiers and env var requirements | completed | Stored in docs/setup_guide.md (includes project ref + DB password). |
